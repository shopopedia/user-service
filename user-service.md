...
- Lifecycle hooks:
  - `@PrePersist` sets `createdAt`, `updatedAt`, normalizes `email`, and defaults `status` to `ACTIVE`
  - `@PreUpdate` updates `updatedAt` and normalizes `email`
...
2. The service normalizes the email to lowercase.
3. The repository checks for an existing record by email.
4. If no record exists, the entity is persisted to the `users` table.
5. `created_at` and `updated_at` are filled automatically.
6. `status` defaults to `ACTIVE` unless explicitly changed later.
